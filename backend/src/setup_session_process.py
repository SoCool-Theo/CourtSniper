...
try:
    from .setup_session import SETUP_ALREADY_RUNNING_EXIT_CODE
    from .setup_session_guard import (
...
class SetupSessionProcessManager:
    ...
    def __init__(
        ...
        *,
        ...
        self._root_dir = Path(root_dir) if root_dir else module_dir.parent
        self._script_path = (
            Path(script_path)
            if script_path
            else module_dir / SETUP_SCRIPT_FILENAME
        )
